...
def predict_frame(model, source, confidence, device):
...
def run_image_display(model, source_path, output_dir, confidence, device, stream_json=False):
    import cv2 as cv

    results = predict_frame(model, str(source_path), confidence, device)
    result = results[0]
...
def run_video_display(model, source_path, output_dir, confidence, device, stream_json=False):
    import cv2 as cv
    ...
    try:
        while capture.isOpened():
            ...
            results = predict_frame(model, frame, confidence, device)
            result = results[0]
...
def run_directory_display(model, source_path, output_dir, confidence, device, stream_json=False):
    import cv2 as cv
    ...
    for image_path in supported_files:
        results = predict_frame(model, str(image_path), confidence, device)
        result = results[0]
        boxes = getattr(result, "boxes", None)
...
def run_display_inference(model, source_path, output_dir, confidence, device, stream_json=False):
    if source_path.is_dir():
        return run_directory_display(model, source_path, output_dir, confidence, device, stream_json)

    if source_path.suffix.lower() in VIDEO_EXTENSIONS:
        return run_video_display(model, source_path, output_dir, confidence, device, stream_json)

    return run_image_display(model, source_path, output_dir, confidence, device, stream_json)


def run_saved_inference(model, source_path, output_dir, confidence, device):
...
def main():
    ...
    run_summary = run_display_inference(model, source_path, output_dir, args.conf, device, args.stream_json) if args.display else run_saved_inference(model, source_path, output_dir, args.conf, device)
